Matriz de Regressão: Topologia HCL & Segurança em Nuvem

Quantificando "Security Smells" em Infraestrutura como Código (IaC) via Análise de AST
...
Aviso de Embargo Acadêmico

Resumo Executivo

Arquitetura do Sistema (Pipeline ELT)

Rigor Metodológico e Estatístico

Matriz de Ground Truth

Estrutura do Repositório

Protocolo de Execução

⚠️ Aviso de Embargo Acadêmico
...
Vetor de Segurança (IDs Checkov)

Preditores Causais Topológicos (AST)

CIS AWS Foundations

Arquitetura e IAM (ex: CKV_AWS_1, CKV_AWS_41)

numLiteralExpression, avgMccabeCC, entropyOfStrings, maxDepthNestedBlocks

PCI DSS v4.0

Redes e Transacional (ex: CKV_AWS_25, CKV_AWS_277)

numDynamicBlocks, numLoops, countOfBooleanFlags, numConditionalExpressions

HIPAA & LGPD

Privacidade e Dados de Saúde (ex: CKV_AWS_5, CKV_AWS_55)
...
Para revisores que utilizam a imagem OCI pré-compilada, a execução do pipeline é abstraída via orquestração declarativa.

Passo 1: Carregar o artefato OCI imutável compilado via Nix:

docker load < result


Passo 2: Executar o pipeline ELT (o docker-compose.yml força nativamente o mapeamento de volumes do host para preservação do estado do DuckDB):
